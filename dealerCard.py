...
This program checks for the best policy when taking into consideratin the
the dealer's card.
...
def writeData(policy,dc, games, deckType, checkHard, stats, wins, winsBj, losses, lossesBj, pushes, pushesBj, mc):
     ...
     filename = "dealCardPolicyTest.csv"
     i = 0
...
if __name__ == '__main__':
    ...
    while games < 1:
        ...
        while deckType < 3:
            ...
            while pPoints < 22:
                faceUpCardPoints = 2
                while faceUpCardPoints < 12:
                    sim(pPoints,faceUpCardPoints,games,deckType,True) 
                    faceUpCardPoints += 1
                pPoints += 1
            deckType += 1
        #q += 1
